...
# 5.3 Print correlations with a few key variables
cat("===== CORRELATION WITH SERUM CREATININE (sc) =====\n")
print(sort(cor_mat[,"sc"], decreasing = TRUE))

cat("===== CORRELATION WITH BLOOD UREA (bu) =====\n")
print(sort(cor_mat[,"bu"], decreasing = TRUE))

# 5.4 Correlation with Target Variable (Classification)
cat("\n===== 5.4 CORRELATION WITH TARGET VARIABLE (CKD) =====\n")

# Convert the factor target variable to a binary numeric vector
ckd_binary <- as.numeric(ckd$classification) - 1 

# Ensure the conversion is correct by checking unique values
cat("Target variable levels: ", paste(levels(ckd$classification), collapse = ", "), "\n")
cat("Target variable min/max (0/1): ", min(ckd_binary), "/", max(ckd_binary), "\n")


# Calculate correlation between numeric features and the binary target
cor_with_target <- apply(ckd[numeric_cols], 2, function(x) cor(x, ckd_binary))
print(sort(cor_with_target, decreasing = TRUE))

# COMMENT (for report):
# - Identify which variables are most correlated with sc, bu, hemo, etc.
# - Mention any potential multicollinearity issues (if strong correlations.
...
# 1. Load the imputed dataset from Member 3
data_file <- "kidney_clean_step3.csv"

if (!file.exists(data_file)) {
  stop(paste(
    "I can't find the file:", data_file,
    "\nMake sure 'kidney_clean_step3.csv' is in this folder."
  ))
} else {
  cat("File found:", data_file, "\n")
}
...
# 2. Recreate numeric and factor column lists
numeric_cols <- c("age","bp","sg","al","su",
                  "bgr","bu","sc","sod","pot",
                  "hemo","pcv","wbcc","rbcc")
...
# 3. Make sure types are correct
ckd[numeric_cols] <- lapply(ckd[numeric_cols], as.numeric)
ckd[factor_cols]  <- lapply(ckd[factor_cols], factor)

cat("\n===== STRUCTURE BEFORE SPLIT =====\n")
str(ckd)
...
ckd_train <- ckd[train_index, ]
ckd_test  <- ckd[-train_index, ]

cat("\n===== TRAIN/TEST SIZES =====\n")
cat("Total rows: ", n, "\n")
cat("Train rows:", nrow(ckd_train), "\n")
cat("Test rows: ", nrow(ckd_test),  "\n")

# 5. Export CSVs for Python
write.csv(ckd,       "kidney_final_full.csv", row.names = FALSE)
write.csv(ckd_train, "kidney_train.csv",      row.names = FALSE)
write.csv(ckd_test,  "kidney_test.csv",       row.names = FALSE)

cat("\n===== FILES EXPORTED =====\n")
cat("Saved 'kidney_final_full.csv'\n")
cat("Saved 'kidney_train.csv'\n")
cat("Saved 'kidney_test.csv'\n")
